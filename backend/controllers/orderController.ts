...
export const createData = async (
  ...
  try {
    const result = await orderService.createService(req.body);
    console.log(result);
    if (result) {
...
export const showData = async (
  ...
  try {
    const result = await orderService.showService();
    if (result) {
      res.status(200).json(result);
    } else {
      res.status(204).json({ message: "No dta found" });
...
export const showIdData = async (
  ...
  try {
    const result = await orderService.showIdService(id);
    console.log(result);
    if (result) {
      res.status(200).json(result);
    } else {
...
export const updateData = async (
  ...
  const { id } = req.params;
  try {
    const result = await orderService.updateService(id, req.body);
    if (result) {
